...
$html = file_get_html(wp_feed, false, $context);
...
libxml_set_streams_context($context);
$rss = simplexml_load_file(wp_feed);
foreach($rss->channel->item as $val){
    $link = strval($val->guid);
    //$content = strval($val->children("content", true));
...
function gEntry($html,$gcontent='')
    {
        $imgarr = [];
        foreach($html->find('figure img') as $iitem) {
            array_push($imgarr, $iitem->getAttribute('data-src'));
            //echo $iitem->getAttribute('data-src').'<br/>';
            //echo $iitem.'<br/>';
            // $attribute = 'data-src';
            // var_dump($iitem->find('img[data-src]')->plaintext);
            // $src = $iitem->src;
