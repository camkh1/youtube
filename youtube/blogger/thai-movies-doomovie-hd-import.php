...
function getByStep($step='')
{
    ...
    switch ($step) {
        case '1':
            ...
            if($dataOne) {
                echo '<script language="javascript" type="text/javascript">window.setTimeout( function(){window.location = "'.base_url.'blogger/thai-movies-doomovie-hd-import.php?step=2&page='.$page.'&cate='.$cate.'";}, 1000 );</script>';
                die;
            }
            break;
        case '2':
            $dataTwo = getTwo($page);
            if($dataTwo) {
                echo '<script language="javascript" type="text/javascript">window.setTimeout( function(){window.location = "'.base_url.'blogger/thai-movies-doomovie-hd-import.php?step=3&page='.$page.'&cate='.$cate.'";}, 1000 );</script>';
                die;
            }
            break;
        case '3':
            $getThree = getThree();
            if($getThree) {
                echo '<script language="javascript" type="text/javascript">window.setTimeout( function(){window.location = "'.base_url.'blogger/thai-movies-doomovie-hd-import.php?step=4&page='.$page.'&cate='.$cate.'";}, 1000 );</script>';
                die;
            }
            break;
        case '4':
               ...
               echo '<script language="javascript" type="text/javascript">window.setTimeout( function(){window.location = "'.base_url.'blogger/thai-movies-doomovie-hd-import.php?page='.$page.'&cate='.$cate.'";}, 3000 );</script>';
                die;
               break;   
        default:
            // code...
...
{
    $file = new file();
    $url = 'https://www.doomovie-hd.com/?r=movie&cate='.$cate.'&page='.$page;
    ...
    foreach ($html->find('section .movie-card') as $m) {
        ...
        if(file_exists($upload_path.$checkForDup)) {
            ...
            if(!empty($SeriesMovies)) {
                if($part <= count($jsonFile->playlist)) {
                    echo 'part '.count($part).' <= list '.count($jsonFile->playlist).'<br/>';
                    continue;
                }
            }
            if($current == $date) {
                ...
                if(!empty($SeriesMovies)) {
                    if(count($vdoInfo->playlist) >= $part) {
                        continue;
                    }   
                }          
                $uniq_id = $vdoInfo->pid;
...
function getFour()
{
    if(file_exists($_SESSION['postFile'])) {
        ...
        if(!empty($jsonFile)) {
            ...
            $sql = "SELECT * FROM  post WHERE p_type='thaimovies' AND yid='$jsonFile->id'";
            $db = new stdClass();
            $db->conn = $conn;
            $db->sql = $sql;
            $data = select($db);
            $title = $jsonFile->titleEn;
            $json = json_encode($jsonFile);
            if(empty($data)) {
                $sql = "INSERT INTO post (`p_name`, `p_conent`, `p_type`, `u_id`) VALUES ('$title', '$json', 'thaimovies','155')";
...
function select($db)
{
  ...
  function insert($db){
    if ($db->conn->query($db->sql) === TRUE) {
      return array('status'=> 'ok','data'=> $db->conn->insert_id);
    } else {
      return array('status'=> 'error','data'=> $db->conn->error);
    }
    $db->conn->close();                               
  }
