...
HTML;
        ...
        } elseif (preg_match('/dropbox/', $content)) {
            foreach ($content->find('.post-body script') as $jsd) {
                if (preg_match('/dropbox/', $jsd->src)) {
                    $string = file_get_contents($jsd->src);
                    if ($string === false) {
                        // deal with error...
                    }
                    $prcot = str_replace('<![CDATA[', '', $string);
                    $prcot = str_replace(']]', '', $prcot);
                    $code  = explode("[", $prcot);
                    $code  = explode("]", $code[1]);
                    $prcot = str_replace(' ', '', $code[0]);
...
HTML;
    ...
    public function searchPost($keyWord='',$bid,$label='',$max=1,$start = 1){
        ...
        if(!empty($label)) {
            ...
            $link_blog = 'https://www.blogger.com/feeds/'.$bid.'/posts/default/-/'.urlencode($label).'?alt=json&max-results='.$max.'&q='.urlencode($keyWord).'&start-index='.$start;
            $arrContextOptions=array(
                "ssl"=>array(
                    "verify_peer"=>false,
                    "verify_peer_name"=>false,
