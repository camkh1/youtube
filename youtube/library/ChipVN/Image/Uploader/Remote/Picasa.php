...
class Remote_Picasa extends Remote
{
    ...
    protected function _doUpload()
    {
        ...
        $this->http->setHeader(array(
            "Authorization: OAuth " . $this->get('cookieLogin'),
            "MIME-Version: 1.0",
        ));
    ...
    public function listAlbums(){
        ...
        $this->http->execute('https://picasaweb.google.com/data/feed/api/user/default', 'GET');

        if ($this->http->errors) {
            $this->_throwHttpError(__METHOD__);
        }
        $xml=simplexml_load_string($this->http->getResponseText());
