...
$bid = !empty($_GET['b']) ? $_GET['b'] : get_post_blog;
$max = !empty($_GET['max']) ? $_GET['max'] : 1;
                                        ...
                                        <?php if(!empty($data)):
                                         ...
                                         foreach ($data->feed->entry as $key => $value):
                                            ...
                                            foreach ($value->category as $categorys) {
                                                ...
                                                if(preg_match('/Thailand/', $categorys->term)) {
                                                    $setLabel = 'thai';
                                                }
                                                if(preg_match('/Cambodia/', $categorys->term)) {
                                                    $setLabel = 'khmer';
                                                }
                                                if(preg_match('/China/', $categorys->term)) {
                                                    $setLabel = 'china';
                                                }
                                                if(preg_match('/Korea/', $categorys->term)) {
                                                    $setLabel = 'Korea';
                                                }
                                                if(preg_match('/India/', $categorys->term)) {
                                                    $setLabel = 'India';
                                                }
                                                if(preg_match('/Hong Kong/', $categorys->term)) {
                                                    $setLabel = 'Hong Kong';
                                                }
                                                if(preg_match('/Taiwan/', $categorys->term)) {
                                                    $setLabel = 'Taiwan';
                                                }
                                                if(preg_match('/Drama/', $categorys->term)) {
                                                    $LabelType = 'Drama';
                                                }
                                                if(preg_match('/Episode/', $categorys->term)) {
                                                    $LabelType = 'Drama';
                                                }
                                                if(preg_match('/On Air/', $categorys->term)) {
                                            ...
                                            <?php
                                                if($key==0) {
                                                    ...
                                                    //Label
                                                    $Cates = $blogger->getLabelBySpec($setLabel ,$LabelType, $LabelStatus);
                                                    //End label
                                                    //echo $Cates;
                                                    //echo @$img;
                                                    $title = $value->title->{'$t'};
                                                    $titles = explode('[', $title);
                                                    if(!empty($titles[0]) && !empty($titles[1])) {
                                                        $title = trim($titles[0]);
                                                    } else {
                                                        $title = explode('|', $title);
                                                        if(!empty($title[0]) && !empty($title[1])) {
                                                            $title = trim($title[0]);
                                                        }
                                                    }

                                                    /*Get list*/
                                                    $list = $blogger->getfromsiteid($link, '', $img, $title, $Cates);
                                                    /*End Get list*/
                                                    ...
                                                    if (!file_exists(dirname(__FILE__) . '/../uploads/posts/'.$_SESSION['user_id'] . '/khmerstation')) {
                                                        mkdir(dirname(__FILE__) . '/../uploads/posts/'.$_SESSION['user_id'] . '/khmerstation', 0700);
                                                    }
                                                    $upload_path = dirname(__FILE__) . '/../uploads/posts/'.$_SESSION['user_id'] . '/khmerstation/';
                                                    $file_name = $list["pid"] . '_' . trim($file_name).'.json';
                                                    ...
                                                    if(!empty($csv)) {
                                                        $setPid = $list["pid"];
                                                        $setpage = $_GET['page'];
                                                        $_SESSION['fsite'] = 'khmerstation';
                                                        echo '<script language="javascript" type="text/javascript">window.setTimeout( function(){window.location = "'.base_url.'/blogger/import.php?item='.$setPid.'&page='.$setpage.'";}, 30 );</script>';
                                                        exit();
                                                    }
                                            ...
                                            <td class=" ">
                                                ...
                                                <ul class="dropdown-menu">
                                                    ...
                                                    <li>
                                                        <a href="<?php echo base_url; ?>blogger/import.php?item=<?php echo @$setPid; ?>&page=<?php echo $_GET['page'];?>"><i class="icon-edit"></i> Post</a>
                                                    </li>
                                                    <li>
                                    ...
                                    <div class="dataTables_footer clearfix">
                                        ...
                                        <div class="col-md-6">
                                            <div class="dataTables_paginate paging_bootstrap">
                                                <?php
                                                $targetpage = base_url . 'blogger/khmerstation.php';
                                                $pagestring = '?page=';
                                                $adjacents = 2;
